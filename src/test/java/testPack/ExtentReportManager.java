...
public class ExtentReportManager implements ITestListener  //or TestListenerAdapter class
{
	...
	public void onStart(ITestContext context) {
		...
		sparkReporter.config().setReportName("Be Cognizant_FaturedNews"); // name of the report
		sparkReporter.config().setTheme(Theme.DARK);		
		extent=new ExtentReports();
		extent.attachReporter(sparkReporter);	
		extent.setSystemInfo("Computer Name","localhost");
		extent.setSystemInfo("Environment","QA");
		extent.setSystemInfo("Tester Name","Jagarapu Ramesh");
	...
	public void onTestSuccess(ITestResult result) {
		...
		File fileobj=((TakesScreenshot)BaseClass.driver).getScreenshotAs(OutputType.FILE);
		try {
			FileUtils.copyFile(fileobj, new File(System.getProperty("user.dir")+"\\screenshots\\"+result.getName()+".png"));
		} catch (IOException e) {
